...
class FlushRPCHandler
    : public RPCResponseHandler,
      private SingleThread {

    GlobalEnvironment& m_environment;
    supercontractserver::Flush m_request;
    std::weak_ptr<VirtualMachineStorageQueryHandler> m_handler;
    std::shared_ptr<AsyncQueryCallback<supercontractserver::FlushReturn>> m_callback;
    std::shared_ptr<AsyncQuery> m_query;

public:
    FlushRPCHandler(GlobalEnvironment& environment,
