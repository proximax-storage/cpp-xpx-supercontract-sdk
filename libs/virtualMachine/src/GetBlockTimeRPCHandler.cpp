...
GetBlockTimeRPCHandler::GetBlockTimeRPCHandler(GlobalEnvironment& environment,
                                                   const supercontractserver::GetBlockTime& request,
                                                   std::weak_ptr<VirtualMachineBlockchainQueryHandler> handler,
                                                   std::shared_ptr<AsyncQueryCallback<supercontractserver::GetBlockTimeReturn>> callback)
        : m_environment(environment), m_request(request), m_handler(std::move(handler)), m_callback(
        std::move(callback)) {}

void GetBlockTimeRPCHandler::process() {
    ...
    handler->blockTime(callback);
}

void GetBlockTimeRPCHandler::onResult(const expected<uint64_t>& res) {
    ...
    supercontractserver::GetBlockTimeReturn status;

    status.set_timestamp(*res);
