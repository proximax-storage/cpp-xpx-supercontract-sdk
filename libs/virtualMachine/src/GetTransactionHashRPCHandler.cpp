...
GetTransactionHashRPCHandler::GetTransactionHashRPCHandler(GlobalEnvironment& environment,
...
void GetTransactionHashRPCHandler::process() {
    ...
    auto[query, callback] = createAsyncQuery<TransactionHash>([this](auto&& res) { onResult(res); },
                                                       [] {}, m_environment, true, true);

    m_query = std::move(query);

    handler->transactionHash(callback);
}

void GetTransactionHashRPCHandler::onResult(const expected<TransactionHash>& res) {
    ...
    supercontractserver::GetTransactionHashReturn status;
