...
PathExistRPCHandler::PathExistRPCHandler(GlobalEnvironment& environment,
...
void PathExistRPCHandler::process() {
    ...
    handler->pathExist(m_request.path(), callback);
}

void PathExistRPCHandler::onResult(const expected<bool>& res) {

    ASSERT(isSingleThread(), m_environment.logger())

    supercontractserver::PathExistReturn status;

    if (res.has_value()) {
        status.set_exist(*res);
    } else {
        status.set_exist(false);
    }
