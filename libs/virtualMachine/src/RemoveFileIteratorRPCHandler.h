...
class RemoveFileIteratorRPCHandler
    : public RPCResponseHandler,
      private SingleThread {

    GlobalEnvironment& m_environment;
    supercontractserver::RemoveDirIterator m_request;
    std::weak_ptr<VirtualMachineStorageQueryHandler> m_handler;
    std::shared_ptr<AsyncQueryCallback<supercontractserver::RemoveDirIteratorReturn>> m_callback;
    std::shared_ptr<AsyncQuery> m_query;

public:
    RemoveFileIteratorRPCHandler(GlobalEnvironment& environment,
