...
class CreateDirRPCHandler
    : public RPCResponseHandler,
      private SingleThread {

    GlobalEnvironment& m_environment;
    supercontractserver::CreateDir m_request;
    std::weak_ptr<VirtualMachineStorageQueryHandler> m_handler;
    std::shared_ptr<AsyncQueryCallback<supercontractserver::CreateDirReturn>> m_callback;
    std::shared_ptr<AsyncQuery> m_query;

public:
    CreateDirRPCHandler(GlobalEnvironment& environment,
