...
GetBlockHeightRPCHandler::GetBlockHeightRPCHandler(GlobalEnvironment& environment,
                                                   const supercontractserver::GetBlockHeight& request,
                                                   std::weak_ptr<VirtualMachineBlockchainQueryHandler> handler,
                                                   std::shared_ptr<AsyncQueryCallback<supercontractserver::GetBlockHeightReturn>> callback)
        : m_environment(environment), m_request(request), m_handler(std::move(handler)), m_callback(
        std::move(callback)) {}

void GetBlockHeightRPCHandler::process() {
    ...
    handler->blockHeight(callback);
}

void GetBlockHeightRPCHandler::onResult(const expected<uint64_t>& res) {
    ...
    supercontractserver::GetBlockHeightReturn status;

    status.set_height(*res);
