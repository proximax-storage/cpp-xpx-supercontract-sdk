...
class GetBlockHashRPCHandler
        : public RPCResponseHandler,
          private SingleThread {

    GlobalEnvironment& m_environment;
    supercontractserver::GetBlockHash m_request;
    std::weak_ptr<VirtualMachineBlockchainQueryHandler> m_handler;
    std::shared_ptr<AsyncQueryCallback<supercontractserver::GetBlockHashReturn>> m_callback;
    std::shared_ptr<AsyncQuery> m_query;

public:
    GetBlockHashRPCHandler(GlobalEnvironment& environment,
...
private:
    void onResult(const expected<BlockHash>& res);
};
