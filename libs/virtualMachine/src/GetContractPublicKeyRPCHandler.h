...
class GetContractPublicKeyRPCHandler
        : public RPCResponseHandler,
          private SingleThread {

    GlobalEnvironment& m_environment;
    supercontractserver::GetContractPublicKey m_request;
    std::weak_ptr<VirtualMachineBlockchainQueryHandler> m_handler;
    std::shared_ptr<AsyncQueryCallback<supercontractserver::GetContractPublicKeyReturn>> m_callback;
    std::shared_ptr<AsyncQuery> m_query;

public:
    GetContractPublicKeyRPCHandler(GlobalEnvironment& environment,
...
private:
    void onResult(const expected<ContractKey>& res);
};
