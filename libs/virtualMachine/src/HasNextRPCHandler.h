...
class HasNextRPCHandler
    : public RPCResponseHandler,
      private SingleThread {

    GlobalEnvironment& m_environment;
    supercontractserver::HasNextIterator m_request;
    std::weak_ptr<VirtualMachineStorageQueryHandler> m_handler;
    std::shared_ptr<AsyncQueryCallback<supercontractserver::HasNextIteratorReturn>> m_callback;
    std::shared_ptr<AsyncQuery> m_query;

public:
    HasNextRPCHandler(GlobalEnvironment& environment,
