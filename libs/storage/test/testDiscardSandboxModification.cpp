...
void onClosedFile(const DriveKey& driveKey,
    ...
    auto [_, callback] = createAsyncQuery<SandboxModificationDigest>([=, &environment, &barrier](auto&& res) {
        ...
        onAppliedSandboxModifications(driveKey, environment, pStorage, barrier, r); }, [] {}, environment, false, true);

    pStorage->applySandboxStorageModifications(driveKey, false, callback);
}
