...
void onCreatedDirectory(const DriveKey& driveKey,
    ...
    auto[__, createDirCallback] = createAsyncQuery<void>([=, &environment, &contextHolder, &barrier](auto&& res) {
        ASSERT_TRUE(res);
...
static const std::string files[11] = {"test/test.txt", "test/test2.txt", "drive/test.txt", "drive/test2.txt",
                                      "mod/test.txt", "mod/test2.txt", "mod/test3.txt", "sc/test.txt",
                                      "mod/gs/test.txt", "mod/gs/test2.txt", "drive/unit/test.txt"};
static int i = 0;
...
void onClosedFile(const DriveKey& driveKey,
    ...
    auto[__, openFileCallback] = createAsyncQuery<uint64_t>([=, &environment, &contextHolder, &barrier](auto&& res) {
        ASSERT_TRUE(res);
...
namespace iterator {
static const std::string expected[17] = {"drive", "test.txt", "test2.txt", "unit", "test.txt", "mod", "gs", "test.txt",
                                         "test2.txt", "test.txt", "test2.txt", "test3.txt", "sc", "test.txt", "test",
                                         "test.txt", "test2.txt"};
static int i = 0;
...
void onIteratorHasEnded(const DriveKey& driveKey,
    ...
    auto[_, callback] = createAsyncQuery<void>([=, &environment, &contextHolder, &barrier](auto&& res) {
        ASSERT_TRUE(res);
        onIteratorDestroyed(driveKey, environment, contextHolder, barrier, id);
    }, [] {}, environment, false, true);
...
void onIteratorHasNext(const DriveKey& driveKey,
                       ...
                       ContextHolder& contextHolder,
                       std::promise<void>& barrier, uint64_t id) {
    auto[_, callback] = createAsyncQuery<DirectoryIteratorInfo>(
            [=, &environment, &contextHolder, &barrier](auto&& res) {
                ASSERT_TRUE(res);
                auto filename = std::move(res->m_name);
                ASSERT_TRUE(filename == expected[i++]);
                onIteratorCreated(driveKey, environment, contextHolder, barrier, id);
            }, [] {}, environment, false, true);
...
void onSandboxModificationsInitiated(const DriveKey& driveKey,
    ...
    auto[_, callback] = createAsyncQuery<uint64_t>([=, &environment, &contextHolder, &barrier](auto&& res) {
        ASSERT_TRUE(res);
        onIteratorCreated(driveKey, environment, contextHolder, barrier, *res);
    }, [] {}, environment, false, true);
...
TEST(Storage, Iterator) {
    ...
    threadManager.execute([&] {
        ...
        auto[_, callback] = createAsyncQuery<std::unique_ptr<StorageModification>>(
                [=, &environment, &contextHolder, &p](auto&& res) {
                    ...
                    createFolders::onModificationsInitiated(driveKey, environment, contextHolder, p);
                }, [] {}, environment, false, true);
    ...
    threadManager.execute([&] {
        auto[_, callback] = createAsyncQuery<std::unique_ptr<StorageModification>>(
                [=, &environment, &contextHolder, &pCreateFile](auto&& res) {
                    ASSERT_TRUE(res);
                    contextHolder.m_storageModification = std::move(*res);
                    createFiles::onModificationsInitiated(driveKey, environment, contextHolder, pCreateFile);
                }, [] {}, environment, false, true);
        contextHolder.m_storage->initiateModifications(driveKey, utils::generateRandomByteValue<ModificationId>(),
                                                       callback);
    });

    ASSERT_EQ(std::future_status::ready, barrierCreateFile.wait_for(std::chrono::seconds(10)));
    ...
    threadManager.execute([&] {
        auto[_, callback] = createAsyncQuery<std::unique_ptr<StorageModification>>(
                [=, &environment, &contextHolder, &pIterate](auto&& res) {
                    ASSERT_TRUE(res);
                    contextHolder.m_storageModification = std::move(*res);
                    iterator::onModificationsInitiated(driveKey, environment, contextHolder, pIterate);
                }, [] {}, environment, false, true);
        contextHolder.m_storage->initiateModifications(driveKey, utils::generateRandomByteValue<ModificationId>(),
                                                       callback);
    });

    ASSERT_EQ(std::future_status::ready, barrierIterate.wait_for(std::chrono::seconds(10)));
