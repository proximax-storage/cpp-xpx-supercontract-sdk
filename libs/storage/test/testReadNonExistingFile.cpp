...
void onFilesystemReceived(const DriveKey& driveKey,
    ...
    FilesystemSimpleTraversal traversal([=, &environment, &contextHolder, &promise](const std::string& path) {
        auto [_, callback] = createAsyncQuery<std::string>([=, &environment, &contextHolder, &promise](auto&& res) { ASSERT_TRUE(res); }, [] {}, environment, false, true);
...
TEST(Storage, ReadNonExisting) {
    ...
    threadManager.execute([&] {
        ...
        auto [_, callback] = createAsyncQuery<std::unique_ptr<StorageModification>>([=, &environment, &contextHolder, &pRead](auto&& res) {
            ...
            onModificationsInitiated(driveKey, environment, contextHolder, pRead); }, [] {}, environment, false, true);
