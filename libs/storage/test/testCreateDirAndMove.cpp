...
void onFileMoved(const DriveKey& driveKey,
    ...
    auto [_, callback] = createAsyncQuery<bool>([=, &environment, &contextHolder, &barrier](auto&& res) {
        ASSERT_TRUE(res);
        onChecking(driveKey, environment, contextHolder, barrier); }, [] {}, environment, false, true);
...
void onCreatedDirectory(const DriveKey& driveKey,
    ...
    auto [_, callback] = createAsyncQuery<void>([=, &environment, &contextHolder, &barrier](auto&& res) {
        ASSERT_TRUE(res);
        onFileMoved(driveKey, environment, contextHolder, barrier); }, [] {}, environment, false, true);
...
TEST(Storage, CreateDirAndMove) {
    ...
    threadManager.execute([&] {
        ...
        auto [_, callback] = createAsyncQuery<std::unique_ptr<StorageModification>>([=, &environment, &contextHolder, &pMove](auto&& res) {
            ASSERT_TRUE(res);
            contextHolder.m_storageModification = std::move(*res);
            move::onModificationsInitiated(driveKey, environment, contextHolder, pMove); }, [] {}, environment, false, true);
        contextHolder.m_storage->initiateModifications(driveKey, utils::generateRandomByteValue<ModificationId>(), callback);
    });
