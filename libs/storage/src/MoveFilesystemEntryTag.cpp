...
MoveFilesystemEntryTag::MoveFilesystemEntryTag(
        ...
        : m_environment(environment), m_request(std::move(request)),
          m_responseReader(stub.PrepareAsyncMoveFilesystemEntry(&m_context, m_request, &completionQueue)),
          m_callback(std::move(callback)) {}

void MoveFilesystemEntryTag::start() {
    m_responseReader->StartCall();
    m_responseReader->Finish(&m_response, &m_status, this);
}

void MoveFilesystemEntryTag::process(bool ok) {
...
void MoveFilesystemEntryTag::cancel() {
    ASSERT(isSingleThread(), m_environment.logger())
