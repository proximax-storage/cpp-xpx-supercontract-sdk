...
namespace sirius { namespace crypto {
	...
	void Sign(const KeyPair& keyPair, const utils::RawBuffer& dataBuffer, Signature& computedSignature) {
		Sign(keyPair, { dataBuffer }, computedSignature);
	}

	void Sign(const KeyPair& keyPair, std::initializer_list<const utils::RawBuffer> buffersList, Signature& computedSignature) {
	...
	bool Verify(const Key& publicKey, const utils::RawBuffer& dataBuffer, const Signature& signature) {
		return Verify(publicKey, { dataBuffer }, signature);
	}

	bool Verify(const Key& publicKey, std::initializer_list<const utils::RawBuffer> buffersList, const Signature& signature) {
